 ...
 * 
 * @param showMiniMap
 * @desc display map when the switch is ON.
 * This setting interlocks with option Display Map.
 * @type switch
 * @default 1
 *
 * @param pixelsPerTile
...
(() => {
    ...
    const DISPLAY_SWITCH_ID = Number(pluginParams.showMiniMap || 1);
    ...
    BattleManager.processVictory = function() {
		...
		if($gameSwitches.value(DISPLAY_SWITCH_ID)){
    ...
    Game_System.prototype.minimapData = function(mapId) {
		if (typeof this._savedMinimapData === 'undefined'){
			//_Game_System_prototype_initialize.call(this);
			this._savedMinimapData = [];
		}
    ...
    Game_Map.prototype.addMinimapData = function(x, y, width, height) {
		...
		if (typeof this._minimapData === 'undefined'){
			this._minimapData = [];
		}
