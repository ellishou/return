...
function isMaxLevel(actor, level) {
	let origin_class_max_level = Number(this._actor.currentClass().meta.F_ChangeMaxLevel)
    //return level >= actor.maxLevel();
	return level >= origin_class_max_level;
};
...
Scene_ClassChange.prototype.onClassChangeOk = function() {
	...
	let origin_class_max_level = Number(this._actor.currentClass().meta.F_ChangeMaxLevel)
	this._actor.changeLevel(origin_class_max_level, false)
...
Windows_SelectClasses.prototype.makeItemList = function() {
    ...
    for (const classInfo of mClassList) {
        // 対象のアクターでない場合
        if (classInfo.actors && classInfo.actors.length && !classInfo.actors.includes(this._actor.actorId().toString())) {
            continue;
        // 対象のスイッチがオンでない場合
        ...
        // 条件を満たすなら追加
		let classInfoData = $dataClasses[classInfo.class]
		classInfoData._filterInfo = classInfo
        //classes.push($dataClasses[classInfo.class]);
		classes.push(classInfoData);
    }
...
Windows_SelectClasses.prototype.drawItem = function(index) {
    ...
    if (classItem) {
        ...
        // 転職可不可の表示制御
        this.changePaintOpacity(isClassEnabled(classItem, this._actor, this._tempActor));
        // 職業名の表示
...
Windows_ClassInfo.prototype.drawAllItems = function() {
    if (this._actor && this._tempActor) {
		...
		//console.log('this._tempActor.index : ', this._tempActor.index)
		//console.log('this._tempActor.classNeed : ', this._tempActor.classNeed)
		//console.log('mClassList : ', mClassList)
		//console.log('currentList : ', currentList)
		
		//console.log('this.itemPadding() :', this.itemPadding())
		//console.log('-this._scrollY + this.itemPadding() :', -this._scrollY + this.itemPadding())
		//console.log('currentList[this._tempActor.index] : ', currentList[this._tempActor.index])
		if(currentList[this._tempActor.index]._filterInfo.ClassInfo){
			const conditions = parseStruct2(currentList[this._tempActor.index]._filterInfo.ClassInfo);
			this.drawTextEx('需要 ' + $dataClasses[conditions[0].Class].name + ' Lv ' + conditions[0].Level, 200, -this._scrollY + this.itemPadding());
		}
		if(currentList[this._tempActor.index]._filterInfo.Item){
			//console.log($dataItems[mClassList[this._tempActor.index].Item])
			this.drawItemName($dataItems[currentList[this._tempActor.index]._filterInfo.Item], 520, -this._scrollY + this.itemPadding());
			this.drawText($gameParty.numItems($dataItems[currentList[this._tempActor.index]._filterInfo.Item]) +'/1'  , 700, -this._scrollY + this.itemPadding())
		}

        // 変更先の職業を表示 aaaaaaaaaaaa
...
Windows_ClassInfo.prototype.drawActorClassLevel = function(x, y) {
    ...
    this.resetTextColor();
	if (this._tempActor.level > Number(this._actor.currentClass().meta.F_ChangeMaxLevel)){
		this.drawText(Number(this._actor.currentClass().meta.F_ChangeMaxLevel), x + 84, y, 36, "right");
	}else{
...
Window_EquipStatus.prototype.drawItem = function(x, y, paramId) {
	...
	if(this._tempActor.level > Number(this._actor.currentClass().meta.F_ChangeMaxLevel)){
		this._actor.changeLevel(Number(this._actor.currentClass().meta.F_ChangeMaxLevel), false)
		this._tempActor.changeLevel(Number(this._actor.currentClass().meta.F_ChangeMaxLevel), false)
	}
    if (this._actor) {
...
Windows_ClassInfo.prototype.isCurrentItemEnabled = function() {
    return isClassEnabled(this.getClass(), this._actor, this._tempActor);
};
 ...
 * ●職業が選択可能かどうか？
 */
function isClassEnabled(item, actor, tempActor) {
	if(tempActor){
		// 判斷當前職業與等級是否符合
		if(currentList[tempActor.index]._filterInfo.ClassInfo){
		//console.log(mClassList[tempActor.index])
			const conditions = parseStruct2(currentList[tempActor.index]._filterInfo.ClassInfo);
			if(actor._classId != conditions[0].Class){
				return false;
			}
			if(actor._level < conditions[0].Level){
				return false;
			}
		}
		
		// 判斷道具是否持有
		if(currentList[tempActor.index]._filterInfo.Item){
			if($gameParty.numItems($dataItems[currentList[tempActor.index]._filterInfo.Item]) < 1){
				return false;
			}
		}
	}
	...
	if(tempActor){
		if(mClassList[tempActor.index].Item){
			$gameParty.loseItem($dataItems[mClassList[tempActor.index].Item], 1);
		}
	}
