...
// ---------------------------------------------------------------------------
// Latest version v1.03 (2021/02/13)
//=============================================================================
/*:ja
@plugindesc ＜メッセージ式アイテム取得インジケータ＞ v1.03
@author 蒼竜
@target MZ
...
@param ObtainedItem_TextColor
@desc メッセージ入手アイテム部分描画色,「文章の表示」の色番号 (default: 0)
@type number
@default 0
...
*/
/*:
@plugindesc <Obtained Item Window Indicator> v1.03
@author Soryu
@target MZ
...
@type string
@param ObtainedItem_TextColor
@desc Color for obtained item text, designated by color code used in "Message" commands. (default: 0)
@type number
@default 0
...
const ObtainedItem_TextColor = Number(Param['ObtainedItem_TextColor'] || 0);
...
Window_Message.prototype.startMessage = function() {
    ...
    if($gameTemp.IsUsingMessageItemIndicator == true) this.SetItemObtainedMessageIndicator();
}
...
Window_Message.prototype.newPage = function(textState) {
    ...
    else if($gameTemp.SetMessageItemIndicator == false) this.ResetDefaultMsgWindowSetting();
...
Window_Message.prototype.SetItemObtainedMessageIndicator = function() {
    ...
    const tt2 = tt.replace(/\x1bC\[(\d+)\]/gi,"");
    this.width = this.textWidth(tt2)+75;
...
Window_Message.prototype.ResetDefaultMsgWindowSetting = function() {
    ...
    $gameTemp.IsUsingMessageItemIndicator = false;
	if(this._ItemIndicator_helpWindow){
...
Window_Message.prototype.updateClose = function() {
    if (this._closing) {
        ...
        if (this.isClosed()) {
            this._closing = false;
            if($gameTemp.BackFromScene_BackLog && $gameTemp.BackFromScene_BackLog==true){;}  //
            else if($gameTemp.ChangedMessageItemIndicator == true) this.ResetDefaultMsgWindowSetting();//
        }
    }
};
...
Game_Interpreter.prototype.command126 = function(params) {
    ...
    const secretItems = [3,4];

	if(SoR_MII_Isopen && value>0){
        if(secretItems.every(v=> v!=$dataItems[params[0]].itypeId)){
...
Game_Temp.prototype.displayItemObtainedMessages = function(item, num, type){
    ...
    const text =  MessageObtained + '「' + "\\I["+icon+"]" + "\\C[" + ObtainedItem_TextColor + "]" +nametxt + "\\C[0]" + '」';
    if(ItemMes_se) AudioManager.playSe(ItemMes_se);
    
    $gameTemp.IsUsingMessageItemIndicator = true;
