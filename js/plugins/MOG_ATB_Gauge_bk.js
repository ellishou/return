...
ImageManager.loadATBIcon = function(filename) {
	...
	let filePath = base + '/img/atb/' + filename + '.png';
	if (fs.existsSync(filePath)) {
		return this.loadBitmap('img/atb/', filename, 0, true);
	} else {
		return this.loadBitmap('img/atb/', 'Enemy_Basic', 0, true);
	}
};
...
Scene_Battle.prototype.update = function() {
	...
	if (this._atbGauge && $gameTemp._refreshATBGauge) {
		this.refreshATBGauge()
	};
};
...
ATB_Gauge.prototype.createIcons = function() {
	...
	for (var i = 0; i < this.battlers().length; i++) {
		...
		if (this._skillIcon) {
			this.createSkillIcon(i, this._icons[i])
		};
	};
};
...
ATB_Gauge.prototype.updateIcon = function(sprite, index) {
	...
	if (this._skillIcon) {
		this.updateSkillIcon(this._skillIcons[index], sprite, battler)
	};
};
...
ATB_Gauge.prototype.updateSkillIcon = function(spriteskill, spriteicon, battler) {
	...
	if (spriteskill.wait > 0) {
		spriteskill.wait--
	};
	spriteskill.opacity = spriteskill.wait > 0 ? 0 : spriteicon.opacity;
	spriteskill.visible = spriteicon.visible;
	if (spriteskill.item != this.item(battler)) {
		this.refreshIconSkill(spriteskill, battler)
	};
};
...
ATB_Gauge.prototype.updateSortY = function() {
	this._iconField.children.sort(function(b, a) {
		return a.y - b.y
	})
};
