...
XdRsData.it.valData = function(index) {
    if (!index || index < 1 || index > 10)     return null;
    if (!this.parameters['variableSet'+index]) return null;
    return JSON.parse(this.parameters['variableSet'+index]);
};
XdRsData.it.setupRegisteredVals = function() {
    this._registeredArr = [];
    this._registeredVals = {};
    for (var i=0;i<10;++i) {
        if (!this.valData(i+1)) continue;
        const valId = +this.valData(i+1).id;
        if (!!valId) {
            this._registeredVals[valId] = i+1;
            this._registeredArr.push(valId);
        }
    }
};
...
XdRsData.it.getValIconIndex = function(valId) {
    const index = this._registeredVals[valId];
    const data = this.valData(index);
    return data ? data.iconIndex : 0;
};
...
XdRsData.it.getItemIconIndex = function(data) {
    ...
    switch(data.type) {
        ...
        case 'gold'     :return +this.parameters['currencyIcon'];
...
//==============================================================================================================
XdRsData.it.Spriteset_Map_createLowerLayer = Spriteset_Map.prototype.createLowerLayer;
Spriteset_Map.prototype.createLowerLayer = function() {
    XdRsData.it.Spriteset_Map_createLowerLayer.call(this);
    this.createInfoController();
};
Spriteset_Map.prototype.createInfoController = function() {
    this._infoController = new Sprite_InfoController();
    this.addChild(this._infoController);
};
Spriteset_Map.prototype.displayInfoTips = function(iconIndex) {
    if (iconIndex > 0) {
        const sprite = this.findTargetSprite($gamePlayer);
