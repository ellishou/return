...
// ♦ ALIAS ♦  addBuff
//==============================
var _alias_mog_bhud_addBuff = Game_Battler.prototype.addBuff
Game_Battler.prototype.addBuff = function(paramId, turns) {
    _alias_mog_bhud_addBuff.call(this,paramId, turns);
...
// ♦ ALIAS ♦  addDebuff
//==============================
var _alias_mog_bhud_addDebuff = Game_Battler.prototype.addDebuff
Game_Battler.prototype.addDebuff = function(paramId, turns) {
    _alias_mog_bhud_addDebuff.call(this,paramId, turns);
	this.need_refresh_bhud_states = true;
};
...
// ♦ ALIAS ♦  eraseBuff
//==============================
var _alias_mog_bhud_eraseBuff = Game_Battler.prototype.eraseBuff
Game_Battler.prototype.eraseBuff = function(paramId) {
...
//==============================
// ♦ ALIAS ♦  updateBuffTurns
//==============================
var _alias_mog_bhud_updateBuffTurns = Game_BattlerBase.prototype.updateBuffTurns
...
//==============================
// ♦ ALIAS ♦  updateStateTurns
//==============================
var _alias_mog_bhud_updateStateTurns = Game_BattlerBase.prototype.updateStateTurns
Game_BattlerBase.prototype.updateStateTurns = function() {
	_alias_mog_bhud_updateStateTurns.call(this);
	this.need_refresh_bhud_states = true;
};

//==============================
// ♦ ALIAS ♦  updateStateSteps
//==============================
var _alias_mog_bhud_updateStateSteps = Game_BattlerBase.prototype.updateStateSteps
Game_BattlerBase.prototype.updateStateSteps = function(state) {
	_alias_mog_bhud_updateStateSteps.call(this, state);
	this.need_refresh_bhud_states = true;
};
...
Battle_Hud.prototype.create_states2 = function() {
	if (String(Moghunter.bhud_states_visible) != "true") {return};
	this.removeChild(this._state_icon);
	this.removeChild(this._state_turn);
	if (!this._battler) {return};
	this._states_data = [0,0,0];
	this._stateIcons = [];
	...
	this.addChild(this._state_icon);
	this._stateTurns = [];
	this._state_turn = new Sprite();
	this._state_turn.x = this._pos_x + Moghunter.bhud_states_pos_x;
	this._state_turn.y = this._pos_y + Moghunter.bhud_states_pos_y;
	this._state_turn.visible = false;
	this.addChild(this._state_turn);
};
...
Battle_Hud.prototype.refresh_states2 = function() {
	...
	for (i = 0; i < this._stateTurns.length; i++){
		this._state_turn.removeChild(this._stateTurns[i]);
	};	
	if (this._battler.allIcons().length == 0) {return};
	this._state_icon.visible = true;
	this._state_turn.visible = true;
	this._stateIcons = [];
	this._stateTurns = [];
	var w = ImageManager.iconWidth;
	var icons = this._battler.allIcons().slice(0,w);
	var turns = this._battler.allStateTurns();
	...
	for (i = 0; i < m; i++){
		 ...
		 //this._stateIcons[i].bitmap.drawText(turns[i], sx, sy, w, w, 'right');
		 this._stateTurns[i] = new Sprite();
		 this._stateTurns[i].setFrame(sx, sy-10, w, w);
		 this._stateTurns[i].bitmap = new Bitmap(512, 640);
		 this._stateTurns[i].bitmap.fontSize = $gameSystem.mainFontSize() - 6
		 this._stateTurns[i].bitmap.textColor = "#FFFFFF";
		 this._stateTurns[i].bitmap.outlineColor = ColorManager.outlineColor;
		 this._stateTurns[i].bitmap.outlineWidth = 3;
		 if (turns[i] != 0){
			this._stateTurns[i].bitmap.drawText(turns[i], sx, sy, w , w , 'right');
		 }
		 
		 if (align === 1) {
		     this._stateIcons[i].x = -((w + 4) * i);
		     this._stateTurns[i].x = -((w + 4) * i);
		 } else if (align === 2) { 
		     this._stateIcons[i].y = (w + 4) * i;
		     this._stateTurns[i].y = (w + 4) * i;
		 } else if (align === 3) {
			 this._stateIcons[i].y = -((w + 4) * i);
			 this._stateTurns[i].y = -((w + 4) * i);
		 } else {	 
		 ...
		 this._state_turn.addChild(this._stateTurns[i]);
	};
};
